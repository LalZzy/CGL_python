...
def cgl_rank(X, triple, lamb, eta, silence=True, tolerence=1e-6, round=10):
    ...
    while True:
        ...
        if (loss_change < tolerence) and (loss_change >= 0):
            print("Stop, iteration: %d, eta: %f B's 'f-norm' decreases: %f, old_loss: %f, current_loss: %f" %
                  (r, eta, loss_change, obj_old, obj))
            print('B change: max={}, min={}'.format(np.max(B), np.min(B)))
            print('regulazition part={}, loss_part={}'.format(regularization_part, loss_part))
            break
        if (r % 10 == 0) and (silence is False):
            print("Iteration: %d, eta: %f B's 'f-norm' decreases: %f, old_loss: %f, current_loss: %f" %
                  (r, eta, loss_change, obj_old, obj))
            print('B change: max={}, min={}'.format(np.max(B), np.min(B)))
            print('regulazition part={}, loss_part={}'.format(regularization_part, loss_part))
        obj_old = obj
...
def cgl_rank_trans(X, triple, lamb, eta, silence=False, tolerence=0.01):
    ...
    while True:
        ...
        if (r % 10 == 0) and (silence == False):
            print("Step: %d, Lambda: %f B's 'f-norm' decreases: %f" %
                  (r, eta, loss_change))
...
def sparse_cgl_rank(X, triple, lamb, eta, silence=False, tolerence=0.00001):
    ...
    while True:
        ...
        for j in range(n):
            ...
            def soft_threshold(x, y): return sgn(x)*max((abs(x) - y), 0)
            tmp = np.array(
                list(map(soft_threshold, tmp, np.zeros_like(tmp)+lamb*eta)))
            tmp.shape = (len(tmp), 1)
        ...
        regularization_part = lamb * np.sum(abs(A))
        loss_part = sum(list(map(loss_func, triple)))
        obj = (regularization_part + loss_part)/(n*p)
        loss_change = obj_old - obj
        if loss_change < tolerence:
