...
def incre_cgl_rank(X, st_idx, tripple, T, T1, T2, T3, A0, lamb, eta, tolerrence=0.00001, silence=False, 
                   update_A1=False):
    '''
...
def test_incre(data_file, link_file, concept_file, file_type, incre_course_num, incre_concept_num, undirect=False,
    ...
    T, T1, T2, T3 = split_tripple(tripple, range(X.shape[0]-incre_course_num,X.shape[0]))
    # 找到一个只有最后一门课才有的词
    X = row_normlize(X)
    # 先训不带增量最后一行的
    # 用全量数据训练模型
    import model
    A0, F0 = model.cgl_rank(X, tripple, lamb=0.01, eta=1,
    ...
    print('\n\n\n')
    A1, F1 = incre_cgl_rank(X, (n_course-incre_course_num, n_concept-incre_concept_num), T, T1, T2, T3, A, eta=5, lamb=0.01,
    ...
    if save:
        np.savetxt('result/ruc_A_whole_{}.txt'.format(file_prefix), A0)
        np.savetxt('result/ruc_A_incre_{}.txt'.format(file_prefix), A1)

        np.savetxt('result/ruc_F_whole_{}.txt'.format(file_prefix), F0)
        np.savetxt('result/ruc_F_incre_{}.txt'.format(file_prefix), F1)
